...
{	
	...
	vec3 ro = vec3(0.+smoothstep(0.,1.,tri(u_time*.6)*2.)*0.1, smoothstep(0.,1.,tri(u_time*3.)*3.)*0.08, -u_time*3.5-140.0);
    ro.y -= camHeight(ro.zx)-.4;
    mo.x += smoothstep(0.7,1.,sin(u_time*.35))*.5-1.5 - smoothstep(-.7,-1.,sin(u_time*.35))*.5;
 
    vec3 eyedir = normalize(vec3(cos(mo.x),mo.y*2.+sin(u_time*.5)*0.1,sin(mo.x)));
    ...
    // Borders...
    float f = smoothstep(0.0, 3.0, u_time)*.5;
