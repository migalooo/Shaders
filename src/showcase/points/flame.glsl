...
void main(  ) {
	...
	float xfuel = max(0.0, (0.5 + u_mouse.y) -abs(2.0*xpart-1.0));//pow(1.0-abs(2.0*xpart-1.0),0.5);

	float timeSpeed = 0.5;
	float realTime = timeSpeed*u_time;
	...
	float sparkRandom = prng(sparkGridIndex) * (1.0 + u_mouse.y * 0.5);
	...
	gl_FragColor = vec4(max(fire,sparks),1.0);
}
